...
class FPOR(base_ALM_const_IL):
    
    def objective(self, s, y):
        return -(s.T@y) / torch.sum(y)

    ## precision
    def metric_constr(self, s, y, alpha):
        return torch.maximum(torch.tensor(0), alpha -(s.T@y) / torch.sum(s))
        # return torch.maximum(torch.tensor(0), alpha*torch.sum(s) -(s.T@y))
    ...
    def eval(self, pred, y):
        ...
        pred = pred.cpu().numpy()
        y_cpu = y.cpu().numpy()
        const = (pred.T@y_cpu)/float(np.sum(pred))
        obj = (pred.T@y_cpu)/float(np.sum(y_cpu))
        return obj, const
    ...
    def log(self, cur_iter):
        ...
        g_objs_s.append(r_obj_s)
        g_const_s.append(r_const_s)
...
if __name__ == "__main__":
    ...
    opt = FPOR(X, y, args.alpha, device, Folding=True, ws=args.ws)
    
    opt.set_workspace(f"logs/{args.ds}/FPOR/{args.seed}/")
